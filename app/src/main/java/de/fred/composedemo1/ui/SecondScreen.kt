...
@Composable
fun SecondScreen() {
    val navigator: de.fred.composedemo1.navigation.Navigator = get()
    val viewModel = getViewModel<SecondScreenViewModel>()
    Column() {
        Text(text = viewModel.getDetailText())
        Button(onClick = { navigator.navigateTo(de.fred.composedemo1.navigation.Navigator.NavTarget.SecondFeature) }){
